...
function extractCartItems(toolResult, plannerParameters) {
    ...
    const parseServiceBlock = (block) => {
        ...
        if (block.multiple_candidates && Array.isArray(block.candidates) && block.candidates.length > 0) {
            const candidates = block.candidates
                .filter(c => c.price > 0)
                .map(c => ({ name: c.name, price: c.price, summary: c.summary, description: c.description }));
    ...
    // Data path: baseTool.execute → { success, data: _run_result }
    //            pricingTool._run → { rawText: actual_pricing, formattedText, service }
    // After PricingTool fix, rawText IS the actual pricing data (candidates/price/etc)
    const pricingRunResult = toolResult.data || toolResult;
    const actualData = pricingRunResult.rawText || pricingRunResult;
